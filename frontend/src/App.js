...
import {
  ...
  Table, TableBody, TableCell, TableHead, TableRow, CircularProgress, Pagination, Box, Paper, IconButton, Chip, Zoom, DialogContentText, Snackbar, Slide, Grow
} from '@mui/material';
...
const BASE_URL = window.env?.REACT_APP_BASE_URL || 'http://localhost:8000/api/v1';
...
export default function TaskManagerApp() {
  ...
  const fetchTasks = useCallback(async (pageNo = 1) => {
    setLoading(true);
    try {
      const res = await axios.get(`${BASE_URL}/tasks?page_no=${pageNo}&page_size=${pageSize}`);
    ...
    <Box className="main-container">
      <Paper elevation={3} className="paper-container">
        <Box className="header-bar">
          <Typography variant="h4" fontWeight={600}>Task Manager</Typography>
          <Box>
            <Button variant="outlined" onClick={() => fetchTasks(page)} startIcon={<RefreshIcon />} sx={{ mr: 1 }}>Refresh</Button>
                ...
                {tasks.map(task => (
                  <Grow in key={task.id} timeout={300}>
                    <TableRow hover className="task-row">
                      ...
                      <TableCell>
                        <IconButton color="error" onClick={() => confirmDeleteTask(task.id)}>
                          <DeleteIcon />
                        </IconButton>
                      </TableCell>
                    </TableRow>
